	...
	export default {
		...
		methods: {
			...
			calculateFreightAmount(seckillId, province, city, area) {
				...
				this.$api.request.calculateSecKillFreightAmount(postData, res => {
					if (res.body.status.statusCode === '0') {
			...
			// 计算同城配送费金额
			calculateDeliveryAmount(seckillId, deliveryLatitude, deliveryLongitude) {
				let that = this;
				let postData = {
					secKillProduct:{
						secKillProductUuid: seckillId
				...
				this.$api.request.calculateSecKillDeliveryAmount(postData, res => {
					if (res.body.status.statusCode === '0') {
			...
			submit() {
				...
				this.$api.request.createSecKillOrder(postData, res => {
					if (res.body.status.statusCode === '0') {
						uni.redirectTo({
