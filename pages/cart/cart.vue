		...
		<view v-else>
			...
			<view class="cart-list">
				<block v-for="(item, index) in cartList" :key="item.shoppingCartUuid">
					<view class="store">
						<image class="portrait" src="/static/image/store-dark.png" mode="aspectFill"></image>&nbsp;&nbsp;&nbsp;{{item.productDTO.merchantDTO.merchantName}}
					</view>
					<view class="cart-item" :class="{'b-b': index!==cartList.length-1}">
						...
						<view class="item-right">
							...
							<text class="price" v-if="item.productDTO.skuEnabled">¥{{item.productSkuDTO.skuUnitPrice}}</text>
							<text class="price" v-if="!item.productDTO.skuEnabled">¥{{item.productDTO.unitPrice}}</text>
							<uni-number-box v-if="!item.productDTO.skuEnabled" class="step" :min="1" :max="item.productDTO.totalUnit" :value="item.unit>item.productDTO.totalUnit?item.productDTO.totalUnit:item.unit"
							 :isMax="item.unit>=item.productDTO.totalUnit?true:false" :isMin="item.unit===1" :index="index" @eventChange="numberChange"></uni-number-box>
							<uni-number-box v-if="item.productDTO.skuEnabled" class="step" :min="1" :max="item.productSkuDTO.skuTotalUnit"
							 ...
							 :isMin="item.unit===1" :index="index" @eventChange="numberChange"></uni-number-box>
						</view>
